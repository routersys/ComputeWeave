...
`IComputeShaderDescriptor<T>.HlslSource` は、生成器がそのシェーダー型に対して書き出したHLSLを返します。出荷の経路はDXCで `cs_6_0` として建てますが、本文は素のHLSLなので、取り出して別の場所で建てられます。自分で用意したDirect3D 11のデバイスのために、FXCへ渡すこともできます。
