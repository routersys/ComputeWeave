...
以上の宣言はアナライザーが検査し、接頭辞 `CMPW` の診断112種類として報告します。対象は属性の位置、ホストとパイプラインメソッドの形、スロットの宣言、資源の契約、生成されるオーバーロードの衝突です。一部にはコード修正が付きます。
...
これらの宣言は、`CMPWD2D` を接頭辞とする100件の診断を報告するアナライザーが検査します。シェーダーは Direct2D が受け付ける DXBC へ FXC でコンパイルします。`d3dcompiler_47.dll` は Windows に同梱されているため、このパッケージはコンパイラーを同梱しません。
...
`IComputeShaderDescriptor<T>.HlslSource` は、生成器がそのシェーダー型に対して書き出したHLSLを返します。出荷の経路はDXCで `cs_6_0` として建てますが、本文は素のHLSLなので、取り出して別の場所で建てられます。自分で用意したDirect3D 11のデバイスもその一つです。

本文はシェーダーモデル6向けに書かれています。古いプロファイルが受け取れるのは、そのプロファイルが備える範囲にシェーダーが収まっている場合だけです。FXCはこれらの多くを建てますが、すべてではありません。入口が掛ける範囲検査の下で到達する群の障壁、全成分を書かない型付きUAVへの書き込み、シェーダーモデル5より後に入った組み込みは、いずれもそこで断られます。もう1つ、FXCの手が届かない形があります。補助型どうしがメンバーメソッドのシグネチャで互いを名指しする形です。どの並びでも解けないため、本文はその型の前方宣言を残します。DXCはそれを受け付けますが、FXCは受け付けません。それ以外の補助型は、それを名指しする宣言より前に宣言され、前方宣言は付きません。どれも本文が述べる束縛の性質ではなく、束縛については本節の残りが述べます。

本文は自分の束縛を述べています。呼ぶ側が満たすべきものは本文に現れます。入口、定数バッファとそれが持つ全フィールド、資源とその割り当てです。生成器から復元しなければならないものは残らないので、本文に対するコンパイラ自身の反射が、束縛の表をそのまま与えます。

| 何を | どこに書かれているか |
|---|---|
| 入口 | `[numthreads(...)]` の印を持つ関数です。名前は `Execute` です。 |
| 群の大きさ | `[numthreads(...)]` が読む3つの定数で、本文の先頭で定義されます。 |
| 定数バッファ | `cbuffer` の宣言が自分の割り当てを持ちます。並びは、宣言したフィールドに対するHLSLの標準の詰め方です。 |
| 資源 | それぞれが自分の割り当てを宣言します。番号を省いた標本器は `s0` に解かれます。 |

形そのものは約束しません。定数バッファが何から始まるか、資源がどの順に割り当てを取るかは版の間で変わりえます。変えた版はその旨をノートに載せます。変わらないのは、本文がそれを述べていることです。建てる本文から束縛を読み取る呼び出し側は版をまたいで正しいままですが、ある版で測った位置を直に書いた呼び出し側はそうなりません。

対となる Direct2D のパッケージは、同じことを別の道筋で満たします。この形をそのまま持ち込まないでください。あちらの画素シェーダーの本文は `d2d1effecthelpers.hlsli` を `#include` で名指しし、入口をそのヘッダーのマクロから組み立てます。捕捉した値を Direct2D が束ねる定数バッファへ集めるのも、そのヘッダーです。本文を建てる呼び出し側にはヘッダーが要りますが、Windows SDK が提供します。捕捉した値そのものは本文に宣言されています。
