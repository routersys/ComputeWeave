...
本文はシェーダーモデル6向けに書かれています。古いプロファイルが受け取れるのは、そのプロファイルが備える範囲にシェーダーが収まっている場合だけです。FXCはこれらの多くを建てますが、すべてではありません。入口が掛ける範囲検査の下で到達する群の障壁、全成分を書かない型付きUAVへの書き込み、シェーダーモデル5より後に入った組み込みは、いずれもそこで断られます。どれも本文が述べる束縛の性質ではなく、束縛については本節の残りが述べます。
